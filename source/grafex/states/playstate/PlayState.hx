...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		vgblack = new FlxSprite().loadGraphic(Paths.image("stupid"));
		vgblack.screenCenter();
		vgblack.cameras = [camBOX];
		add(vgblack);

		rippl = new FlxSprite(0, 0);
		rippl.frames = Paths.getSparrowAtlas('Ripple splashscreen');
		rippl.antialiasing = ClientPrefs.globalAntialiasing;
		rippl.animation.addByPrefix('speeeen', 'ripple splashscreen', 24, true);
		rippl.animation.play('speeeen');
		rippl.updateHitbox();
		rippl.screenCenter();
		rippl.alpha = 0;
		add(rippl);
		...
		scoreTxt = new FlxText(healthBarBG.x - healthBarBG.width / 2, healthBarBG.y + 28, 0, "", 20);
		scoreTxt.setFormat(Paths.font("Robot_Font.otf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 3;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		missTxt = new FlxText(scoreTxt.x, scoreTxt.y - 46, 0, "", 20);
		missTxt.setFormat(Paths.font("Robot_Font.otf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("Robot_Font.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		sub.setFormat(Paths.font('Robot_Font.otf'), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
        sub.size = 32;
		sub.borderSize = 1.25;
		sub.scrollFactor.set();
		add(sub);

        for (object in [strumLineNotes, grpNoteSplashes, notes, healthBar, healthBarWN, healthBarBG, iconP1, iconP2, scoreTxt, missTxt, botplayTxt, doof, sub, rippl]) object.cameras = [camHUD];
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				//case 'ripple': startDialogue(dialogueJson);
				default:
					startCountdown();
			}
			seenCutscene = true;
	...
	{
		scoreTxt.text = 'Health: ' + Math.fround((health * 100) /2);
    ...
    public function triggerEventNote(eventName:String, value1:String, value2:String, ?value3:String) {
		switch(eventName) {
			...
			case 'Flash Camera':
				var color:FlxColor = Utils.returnColor(value1);
				var time:Float = Std.parseFloat(value2);
	...
	override function stepHit()
	{
		...
		switch(curStep) {
			case 916:
				var epicShader:FlxRuntimeShader = new FlxRuntimeShader(File.getContent(Paths.shaderFragment('chrom')));
		...
		if (curStep == 1)
			FlxTween.tween(rippl, {alpha: 1}, 1.3, {ease: FlxEase.sineInOut});

		if (curStep == 32)
			FlxTween.tween(rippl, {alpha: 0}, 1.3, {ease: FlxEase.sineInOut});
